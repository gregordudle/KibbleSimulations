model Coil "Coil"
  ...
  Modelica.Electrical.Analog.Interfaces.Pin aux_a annotation(
    Placement(transformation(origin = {150, -82}, extent = {{-10, -10}, {10, 10}}), iconTransformation(origin = {150, -82}, extent = {{-10, -10}, {10, 10}})));
  Modelica.Electrical.Analog.Interfaces.Pin aux_b annotation(
    Placement(transformation(origin = {152, -122}, extent = {{-10, -10}, {10, 10}}), iconTransformation(origin = {152, -122}, extent = {{-10, -10}, {10, 10}})));
...
protected 
  ...
  // the center of the coil is at pos_z
  //    the upper wires are at pos_z + coil_height/2
  //    the lower wires are at pos_z - coil_height/2
  ...
  function voltage "Returns the voltage induced in a coil that moves with velocity v, at position pos_z, in a flux density B which has finite extension in space."
  // the center of the coil is at pos_z
  //    the upper wires are at pos_z + coil_height/2
  //    the lower wires are at pos_z - coil_height/2
    ...
    input Modelica.Units.SI.Length coil_height;
    input Modelica.Units.SI.Length coil_length;
    output Modelica.Units.SI.Voltage voltage;
    
    
    protected
      Modelica.Units.SI.Length field_height = 0.05 "B is different from zero in [-field_height,+field_height]";
      Modelica.Units.SI.MagneticFluxDensity B = 1 "Flux density B in Tesla";
      Modelica.Units.SI.Voltage v_upper;
      Modelica.Units.SI.Voltage v_lower;
    algorithm
      v_upper := if (pos_z + coil_height/2) >= -field_height and (pos_z + coil_height/2) <= field_height then coil_length * B * velo else 0;
      v_lower := if (pos_z - coil_height/2) >= -field_height and (pos_z - coil_height/2) <= field_height then coil_length * B * velo else 0;
      voltage := -v_upper + v_lower;
  end voltage;
  ...
  equation
    ...
    aux_a.i + aux_b.i = 0;
    aux_a.v - aux_b.v = voltage(pos.s,der(pos.s),coil_h,coil_l);
    annotation(
